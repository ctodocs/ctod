...
>On a Jekyll website, the title in the front matter is interpreted as a level 1 heading.
>Only use level 2 or lower headings in your document.
...
#### How to Create Paragraphs with Markdown
...
#### How to Create Lists with Markdown
...
1. Update `index.markdown` to describe what you are documenting.
  *. Add one level-2 header.
  *. Add a paragraph under each header.
  *. Add one link to an external website.
...
Headings, paragraphs, and lists form the bulk of documentation. Markdown can also be used to create links and code sections.
...
* To link to a page within your site, use the absolute path from the root of your website folder, `/index.markdown`
...
#### How to Create Code Sections in Markdown
...
2. Copy the draft document you created previously into the new document and add markdown formatting as needed.
3. Copy-paste the following Jekyll frontmatter to the top of the new document. Adjust the title.
...
As you refine your documentation, you might need additional features beyond headings, paragraphs, lists, links, and code.
...
* {% raw %}A curly brace with percent signs `{% %}` is used for executable statements. For example, `{% highlight ruby %} ... {% endhighlight %}` is a plugin that adds ruby syntax highlighting to a code block. {% endraw %}

These features are good for extending the functionality of your documentation, but they should be explored after completing the core content of your documentation.

### Using HTML

Jekyll will also render any valid HTML in a markdown document.
It is good practice to use Markdown as much as possible to simplify writing and maintenance.
But, you may have instances where you need specific features from HTML.

#### How to Use HTML with Markdown
